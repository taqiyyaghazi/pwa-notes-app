...
const PushNotificationHelper = {
    ...
    async _initialServiceWorker() {
        ...
        if (!this._registrationServiceWorker) {
            ...
            window.alert(
                'Service worker is not registered yet in this browser or not supported'
            );
        }
    },
    ...
    async _subscribePushMessage(event) {
        ...
        try {
            ...
            const response = await PushNotificationAPI.subscribe(
    ...
    async _unsubscribePushMessage(event) {
        ...
        console.log(
            '_unsubscribePushMessage: Unsubscribing to push message...'
        );
        let pushSubscription = null;
        try {
            pushSubscription =
            ...
            const response = await PushNotificationAPI.unsubscribe(
                pushSubscription.toJSON()
            );
    ...
    _generateSubscribeOptions() {
        return {
            ...
            applicationServerKey: convertUrlB64ToUint8Array(
